...
import { loginApi, getMe, loginADApi } from '../auth/useLogin';
...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (email, password) => {
    setIsLoading(true);
    try {
      if (email === 'superadmin@yourdomain.com') {
        const res = await loginApi(email, password);
        
        localStorage.setItem('token', res.token);
        console.log("Local Login Response User:", res.user);
        setUser(res.user);
      } else {
        const res = await loginADApi(email, password);
        console.log("AD Login Response User:", res);
        localStorage.setItem('token', res.token);
        console.log("AD Login Response User:", res.user);
        setUser(res.user);
      }
    } catch (error) {
